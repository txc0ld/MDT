...
The domain should target intent through page architecture, not exact-match naming.

Primary query cluster:

- bedtime stories for kids
- read aloud bedtime stories
- calm bedtime stories for children
- bedtime stories for 3 year olds
- bedtime stories for 5 year olds
- short bedtime stories for toddlers
- parent read aloud stories
- illustrated bedtime story app
...
- Point `minidreamtime.com` and `www.minidreamtime.com` to the hosting provider.
- Choose one canonical host, probably `https://minidreamtime.com`.
- Redirect `www` to root, or root to `www`, but do not serve both.
- Add HTTPS.
- Add canonical tags.
- Add `robots.txt` and sitemap.
- Add schema: `Organization`, `WebSite`, and eventually `Product` / `SoftwareApplication`.
- Add `llms.txt` and `pricing.md` before public launch.
